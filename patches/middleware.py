...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    # Merge model-level toolIds (from model config) with user-selected tool_ids from the client.
    # This mirrors the skillIds server-side merge below and ensures tools configured on a model
    # are always active without requiring the user to manually select them in the chat UI.
    user_tool_ids = list(form_data.pop("tool_ids", None) or [])
    model_tool_ids = list(model.get("info", {}).get("meta", {}).get("toolIds", []))
    all_tool_ids = list(dict.fromkeys(user_tool_ids + model_tool_ids))  # dedup, preserve order
    tool_ids = all_tool_ids if all_tool_ids else None
    log.info(f"[patch] tool_ids merged: user={user_tool_ids} model={model_tool_ids} final={tool_ids}")
